...
//static unsigned int test_memory[10];
...
int main (int argc, char **argv)
{
  ...
  */
  ...
  double *lyst = (double *) malloc(NUM*sizeof(double));
	//double *lyst = (double *) dmalloc(NUM*sizeof(double));
	//double lyst[NUM];
  ...
  // Uncomment here for parallel quicksort
...
int partition(double lyst[], int lo, int hi)
{
  ...
  // make it deterministic always cut in half
  int r = (int) (lo + (hi-lo + 1) * 0.5);
	//printf("Index: %d \t Size: %lu\n", r, sizeof(lyst)/sizeof(lyst[0]));
	double pivot = lyst[r];
	swap(lyst, r, hi);
	for (int i = lo; i < hi; i ++)
	{
